...
namespace Einsteins_Riddle
{
   public enum Colour { Red, Green, White, Yellow, Blue }
    public enum Nationality { Brit, Swede, Dane, Norwegian, German }
    public enum Pet { Dog, Birds, Cats, Horse, Fish }
       ...
       static Dictionary<Type, Func<House, dynamic, bool>> _eFn = new Dictionary<Type, Func<House, dynamic, bool>>
        ...
        static LinqNoPerm()
        {
            var candidates =
                ...
                where (colours == Colour.Red) == (nations == Nationality.Brit) //r2
        ...
        {
            var owner = _solved.Where(h=>h.P==Pet.Fish).Single().N;
            WriteLine($"The fish owner is {owner}");
            Write(ToString());
            Read();
        }
    }
